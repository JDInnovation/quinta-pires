    ...
    properties: {
      ...
      customer: {
        ...
        properties: {
          ...
          addressRaw: { type: ["string", "null"] },
          nifRaw: { type: ["string", "null"] },
...
async function callOpenAi(env: Env, payload: AnalyzePayload): Promise<unknown> {
  ...
  const systemPrompt = [
    ...
    "Delivery-time instructions: format as 'Entrega após as HH:MMH' (only after a time) or 'Entrega até as HH:MMH' (only until a time). Example: 'só posso depois das 18' -> 'Entrega após as 18:00H'; 'só até as 16' -> 'Entrega até as 16:00H'.",
    "Product-specific requests about ripeness/color/state go to notes with the product and qualifier capitalized, e.g. 'quero tomate maduro' -> 'Tomate Coração de Boi Maduro'; 'quero banana verde' -> 'Banana Verde'.",
    "Put each note as a separate string in the generalNotes array. If there are no such instructions, generalNotes must be an empty array.",
